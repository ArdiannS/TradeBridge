...
class UserModel{
    ...
    static async addUser(username, password, email, birthday, usertype,res) {
      ...
      static async UserLogIn(username, password, res) {
        return new Promise(resolve => {
          database.query("SELECT * FROM Users WHERE username = ?", [username], (error, result) => {
            ...
            if (result.length === 0) {
              ...
              const existsTheUsername = "SELECT passwordi FROM Users WHERE username = ?";
      ...
      static async deleteUser(id){
        return new Promise(resolve =>{
          database.query("Delete From Users where userid = ?",[id],(err,result)=>{
            if(err){
              return console.error(err.message);
            }
            resolve(result);
